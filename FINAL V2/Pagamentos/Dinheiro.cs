    ...
    public partial class Dinheiro : Form
    {
        ...
        private static void SendEmailWithGmailAPI(string attachmentPath, string Destinatario)
        {
            ...
            using (var stream = new FileStream("client_secret.json", FileMode.Open, FileAccess.Read))
            {
                ...
                if (System.Text.RegularExpressions.Regex.IsMatch(Destinatario, emailPattern, System.Text.RegularExpressions.RegexOptions.IgnoreCase))
                {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Por favor, insira um valor.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);

                // Retorna o foco para o TextBox1
                textBox3.Focus();
                return; // Sai do método para impedir a execução das outras funções
            }
        ...
        private void GerarNotaFiscal(string caminhoCompleto)
        {
            ...
            XmlElement dinheiroTotalElement = xmlDoc.CreateElement("Dinheiro");
            dinheiroTotalElement.InnerText = "R$ " + textBox3.Text;
            root.AppendChild(dinheiroTotalElement);

            XmlElement trocoTotalElement = xmlDoc.CreateElement("Troco");
            trocoTotalElement.InnerText = button2.Text;
            root.AppendChild(trocoTotalElement);

            string metodoPagamento = "Dinheiro";
            XmlElement Metodo = xmlDoc.CreateElement("Metodo_de_Pagamento");
            Metodo.InnerText = metodoPagamento;
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(textBox3.Text))
            {
                // Converter o valor do TextBox3 para float
                if (float.TryParse(textBox3.Text, out float valorTextBox))
                {
                    // Converter o valor formatado de vendas para float
                    string valorFormatado = valorFormatadoVendas; // Remover ".Text" se valorFormatadoVendas já é uma string
                    float valorFormatadoFloat;
                    if (float.TryParse(valorFormatado, out valorFormatadoFloat))
                    {
                        // Arredondar o valor formatado para duas casas decimais
                        valorFormatadoFloat = (float)Math.Round(valorFormatadoFloat, 2);

                        // Calcular a diferença
                        float diferenca = valorTextBox - valorFormatadoFloat;

                        // Exibir a diferença no Label7
                        button2.Text = "R$ " + diferenca.ToString("F2");
                    }
                    else
                    {
                        // Se a conversão do valor formatado falhar, exibir uma mensagem de erro ou tratar a situação adequadamente
                        
                    }
                }
                else
                {
                    // Se a conversão do TextBox3 falhar, exibir uma mensagem de erro ou tratar a situação adequadamente
                    
                }
            }
            else
            {
                // Se o TextBox3 estiver vazio, exibir uma mensagem de erro ou tratar a situação adequadamente
